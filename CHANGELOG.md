...
- **Trustworthy characteristic matching** — `vair.py` gains `TrustworthyCharacteristic` dataclass and `match_trustworthy_characteristics()` function. 11 characteristics from ISO/IEC TR 24028:2020 and EU AI Act Art. 9-15 (accuracy, robustness, cybersecurity, transparency, fairness, privacy, safety, accountability, controllability, reliability, resilience). Inferred from VAIR type matches with keyword fallback heuristics. 16 new tests.
- **Trustworthy characteristics in build_landscape** — `_vair_enrich()` now calls `match_trustworthy_characteristics()` to populate `RiskCard.trustworthy_characteristics` during landscape construction. Characteristics derived from VAIR type inference + keyword heuristics without LLM calls. 3 new tests.
