...
- **Stakeholder trustworthy interests** — `Stakeholder.trustworthy_interests` populated during entity enrichment (ingest pass 4) by normalizing free-text interests against ISO/IEC 24028 canonical characteristics via keyword heuristics from `vair.py`. Extended accuracy and fairness keyword lists to cover positive forms ("accura", "fair", "equitab", "non-discriminat"). 3 new tests.
