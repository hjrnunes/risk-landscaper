...
- **Stakeholder trustworthy interests** — `Stakeholder.trustworthy_interests` populated during entity enrichment (ingest pass 4) by normalizing free-text interests against canonical characteristics via keyword heuristics. Extended accuracy and fairness keyword lists to cover positive forms. 5 new tests.
- **Directional trustworthy characteristics in report** — Risk Card report section 6 now shows which stakeholders have declared interest in each threatened characteristic, making the Lewis et al. directionality (organization exhibits characteristic toward stakeholders) visible in the output.
